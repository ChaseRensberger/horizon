...
func AddRSSSnapshotToDatabase(rssVideoSnapshot *models.RSSVideoSnapshot, mongoClient *mongo.Client) error {
	collection := mongoClient.Database(config.MongoDatabase).Collection("rss_video_snapshots")
	...
	fmt.Printf("Inserted new rss video snapshot with ID: %s", res.InsertedID)
